...
async function loadAggregate(manifestPath) {
  let source;
  try {
  ...
  assert(
    !Object.hasOwn(aggregate, "site"),
    `${manifestPath} still holds a site mapping; move it to site.yaml, or rerun without --update to replace the file`,
  );
  const unknown = Object.keys(aggregate).filter((field) => field !== "projects");
  ...
  return { projects: validateProjectList(aggregate.projects, `${manifestPath}.projects`) };
}
...
function stringifyAggregate(projects) {
...
export async function rebuildManifest({
  ...
  loaded.sort((left, right) => bySlug(left.project, right.project));
  const scanned = validateProjectList(loaded.map(({ project }) => project), "scanned projects");
  const existing = update ? await loadAggregate(manifestPath) : null;
  const projects = (existing ? mergeProjects(existing.projects, scanned) : scanned)
    .slice()
    .sort(bySlug);
  ...
  const manifestChanged = await writeIfChanged(manifestPath, stringifyAggregate(projects));
...
project list; --update preserves entries that the scan did not match. The
manifest is always stored in canonical slug order; display order comes from
the order list in site.yaml and is applied by npm run build.`);
}
